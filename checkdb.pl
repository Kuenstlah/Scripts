...
if ($mode eq "spawn") {
	...
	$empf="PoGo-Ticker";
	if (($hour >= 0 && $hour <= 8) && ( $day==1 || $day==2 || $day==3 || $day==4 || $day==5) ) {
		# Ticker shows only rare Pokemon during night and at work days
		$pokemon_ids="3,6,9,38,45,51,57,59,62,65,68,71,76,80,82,83,103,106,107,108,110,112,113,114,115,128,130,131,132,134,135,136,139,141,142,143,144,145,146,149,150,151";
	} else {
		$pokemon_ids="3,6,9,24,26,28,31,34,36,38,40,45,51,53,57,59,62,65,68,71,76,78,80,82,83,85,87,89,91,94,95,101,103,106,107,108,110,112,113,114,115,122,123,125,127,128,130,131,132,134,135,136,137,139,141,142,143,144,145,146,148,149,150,151";
	}
}
...
if ($mysql) {
        $db = DBI->connect("DBI:mysql:pogo:localhost","root","root") or die "Connection Error: $DBI::errstr\n";
} else {
